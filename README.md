...
SearchBenchmarks collects a couple of search algorithms and evaluates them in a simple benchmark framework. Technically, we talk about search for a byte sequence, but in julia, it also covers searching for substrings in strings. String search is widely used, its performance can have substantial impact on application performance.
...
You are kindly invited to participate. I will add all search functions to the package which are sent to me and pass my package tests.

## Current State

### technical

Package compiles, passes tests and builds at least with Windows 10 and Julia 1.4.1. I have no idea why some CI tools report "unable to find commit in GitHub" or "build failed". If someone has an idea - please help. I would really like to get CI tools running as expected, but I am new to GitHub and linked CI tools  - this is my first Julia package in a "ready to use" state (I would not claim release 1.0, though).
...
I took a closer look at the julia implementation. I knew about bloom filters, but in a scenario of searching for the first match from a list of search phrases. Usage for simple search was new to me, and a quick web search did not reveal other cases using bloom filters for simple search. Seems to be not that common.
...
Benchmark results for random data were really surprising to me: the RND plot shows an unexpected local maximum for "bloom test first" at a pattern length of 32. I found no explanation for that, so far. Comments are welcome.
